...
- `src/components/Header.tsx` — "Himla" + location with pin icon + search icon that toggles a glass-card search field. When search opens with no query, shows recent locations with clock icons. Search results overlay content (absolute positioned). Click outside closes search.
- `src/components/CurrentWeatherCard.tsx` — Current temperature, weather icon with tooltip, condition label, feels-like
- `src/components/WeatherIcon.tsx` — Maps WMO icon name strings to Lucide components. Supports `isNight` prop to swap Sun→Moon and CloudSun→CloudMoon. Supports `tooltip` prop for hover text.
